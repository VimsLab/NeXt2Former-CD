...
class ChangeDataset(data.Dataset):
    def __init__(self, setting, split_name, preprocess=None):
        ...
        self._use_weather_data = None
        self._weather_data_format = None

        if 'use_weather_data' in setting and setting['use_weather_data']:
            self._use_weather_data = setting['use_weather_data']
            self._weather_data_format = setting['weather_data_format']
    ...
    def __getitem__(self, index):
        ...
        if self._use_weather_data:
            intresting_variables = [# Thermodynamic State Variables
                        'TSurfStd', # surface skin temperature
                        'TSurfAir', # surface air temperature
                        # 'TAirStd', # standard air temperature
                        'PSurfStd', # surface pressure

                        # Water Vapor Variables
                        # 'H2OMMRStd', # standard water vapor mixing ratio
                        'H2OMMRSurf', # surface water vapor mixing ratio
                        'totH2OStd', # total water vapor column
                        'RelHumSurf', # surface relative humidity

                        # Cloud Variables
                        'CldFrcTot', # total cloud fraction

                        # Ozone Variables
                        'totO3Std', # total ozone column.

                        # Carbon Dioxide Variables
                        # 'CO_total_column' # total column of carbon monoxide
                        ]
            A_weather_path = os.path.join(self._root_path, 'A', item_name + self._weather_data_format)
            B_weather_path = os.path.join(self._root_path, 'B', item_name + self._weather_data_format)
            A_weather = netCDF4.Dataset(A_weather_path, 'r')
            B_weather = netCDF4.Dataset(B_weather_path, 'r')
            
            for var in intresting_variables:
                if var in A_weather.variables and var in B_weather.variables:
                    A_var = A_weather.variables[var][:]
                    B_var = B_weather.variables[var][:]
                    
                    if A_var.shape != B_var.shape:
                        raise ValueError(f"Variable {var} has different shapes in A and B: {A_var.shape} vs {B_var.shape}")
                    A = np.concatenate((A, A_var[..., np.newaxis]), axis=-1)
                    B = np.concatenate((B, B_var[..., np.newaxis]), axis=-1)
 
                else:
                    raise KeyError(f"Variable {var} not found in one of the datasets.")
