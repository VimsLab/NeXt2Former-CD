...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.expand(x) # B, H, W, C*(scale^2) or identity
        B, H, W, C = x.shape
        if self.dim_scale > 1:
            x = rearrange(
                x,
...
class UpsampleExpand(nn.Module):
    def __init__(self, input_resolution, dim, patch_size=4, norm_layer=nn.LayerNorm, out_dim=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class Mamba_up(nn.Module):
    def __init__(self, dim, input_resolution, depth, dt_rank="auto", 
                 ...
                 shared_ssm=False, softmax_version=False, upsample_dim=None,
...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        if in_channels is None:
            in_channels = [embed_dim * (2 ** i) for i in range(self.num_layers)]
        if len(in_channels) < self.num_layers:
            raise ValueError("in_channels must have at least as many entries as depths.")
        self.stage_channels = list(in_channels)
        ...
        for i_layer in range(self.num_layers):
            ...
            curr_dim = int(self.stage_channels[stage_index])
            next_dim = int(self.stage_channels[stage_index - 1]) if stage_index - 1 >= 0 else curr_dim
            if i_layer == 0:
                ...
                layer_up = Mamba_up(dim=curr_dim,
                                         input_resolution=(
                                         ...
                                         upsample_dim=next_dim if (i_layer < self.num_layers - 1) else None,
                                         use_checkpoint=use_checkpoint)
            self.layers_up.append(layer_up)

        self.norm_up = norm_layer(self.stage_channels[0])
        if self.deep_supervision:
            self.norm_ds = nn.ModuleList([norm_layer(self.stage_channels[self.num_layers - 2 - i_layer]) for i_layer in range(self.num_layers-1)])
            self.output_ds = nn.ModuleList([nn.Conv2d(in_channels=self.stage_channels[self.num_layers - 2 - i_layer], out_channels=self.num_classes, kernel_size=1, bias=False) for i_layer in range(self.num_layers-1)])
        ...
        self.up = FinalUpsample_X4(input_resolution=(img_size[0] // patch_size, img_size[1] // patch_size),
                                      patch_size=4, dim=self.stage_channels[0])
        self.output = nn.Conv2d(in_channels=self.stage_channels[0], out_channels=self.num_classes, kernel_size=1, bias=False)
