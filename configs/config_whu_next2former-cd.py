...
C.mask2former_class_weights = [1.0, 3.0]  # e.g., [1.0, 5.0]
C.mask2former_class_weight = 1.0
C.mask2former_dice_weight = 1.5
C.mask2former_mask_weight = 1.0
C.mask2former_no_object_weight = 0.05
C.mask2former_num_points = 16384
C.mask2former_oversample_ratio = 3.0
C.mask2former_importance_sample_ratio = 0.85
...
C.log_dir = osp.abspath('log_paper_final_8bs/log_WHU/' + 'log_' + C.dataset_name + '_' + backbone_str + '_' + C.decoder)
...
C.test_log_file = C.log_dir + '/test_' + exp_time + '.log'
C.link_test_log_file = C.log_dir + '/test_last.log'
