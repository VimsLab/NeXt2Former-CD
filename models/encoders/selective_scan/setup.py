...
def get_ext():
    ext_modules = []
    cc_flag = []
    bare_metal_version = Version("0")
    conda_prefix = os.environ.get("CONDA_PREFIX")
    extra_link_args = []
    if conda_prefix:
        # Prefer active conda env libs instead of a hardcoded machine-specific path.
        extra_link_args.extend([f"-L{conda_prefix}/lib", f"-L{conda_prefix}/lib64"])
