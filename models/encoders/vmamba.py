    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            if pe_key in state_dict:
                ckpt_in_chans = state_dict[pe_key].shape[1]
                print(f"'{pe_key}' shape in checkpoint: {state_dict[pe_key].shape}")
                if ckpt_in_chans != self.in_chans:
                    print(f"Mismatch in input channels (ckpt: {ckpt_in_chans}, model: {self.in_chans})")
                    print(f"Initializing '{pe_key}' in state_dict with random weights to match input channels.")
                    state_dict[pe_key] = torch.randn(
                        (state_dict[pe_key].shape[0],
                         self.in_chans, 
                         state_dict[pe_key].shape[2], 
                         state_dict[pe_key].shape[3]), dtype=state_dict[pe_key].dtype
                    )
            print("==================================================")
