...
"""Dataset Path"""
C.dataset_name = 'CDD'
C.root_folder = osp.join(os.environ.get('DATASETS_ROOT', './datasets'), 'CDD-256')
C.A_format = '.jpg'
C.B_format = '.jpg'
C.gt_format = '.jpg'
C.is_test = False
C.num_train_imgs = 10000
C.num_eval_imgs = 2998  # missing one file in the dataset
C.num_classes = 2
...
C.mask2former_class_weights = [1.0, 1.5]  # e.g., [1.0, 5.0]
...
"""Store Config"""
C.checkpoint_start_epoch = 2  # scaled from 5 to 2
C.checkpoint_step = 2  # scaled from 5 to 2
...
C.log_dir = osp.abspath('log_paper_final_8bs/log_CDD/' + 'log_' + C.dataset_name + '_' + backbone_str + '_' + C.decoder)
