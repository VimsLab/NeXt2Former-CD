...
def print_iou(iou, recall, precision, freq_IoU, mean_pixel_acc, pixel_acc,
                dice,
    ...
    if show_no_back:
        lines.append('----------     %-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%' % ('mean_IoU', mean_IoU * 100, 'precision', precision, 'recall', recall, 'f1', f1,
                                                                                                                'mean_IU_no_back', mean_IoU_no_back*100, 'freq_IoU', freq_IoU*100, 'mean_pixel_acc', mean_pixel_acc*100, 'pixel_acc',pixel_acc*100, 'dice', dice*100))
    else:
        lines.append('----------     %-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%\t%-8s\t%.3f%%' % ('mean_IoU', mean_IoU * 100, 'precision', precision, 'recall', recall, 'f1', f1, 'freq_IoU', freq_IoU*100, 
                                                                                                    'mean_pixel_acc', mean_pixel_acc*100, 'pixel_acc',pixel_acc*100, 'dice', dice*100))
    line = "\n".join(lines)
    if not no_print:
        print(line)
    return line
